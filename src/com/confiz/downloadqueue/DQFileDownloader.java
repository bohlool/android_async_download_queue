...
import com.confiz.downloadqueue.interfaces.DQRequestHanlder;
...
public class DQFileDownloader extends Thread {
	...
	public DQFileDownloader(Context context, DQRequestHanlder listener, DQRequest data, DQResponseListener queueListener) {
	...
	protected void onPostExecute(Boolean downloadCompleted) {
		...
		try {
			...
			// final boolean success = fileForRename.renameTo(new
			// File(this.downloadingFileDirectory,
			...
			// } else {
			//
			// if (whichMessageToDisplay == DQErrors.NO_ERROR && request.currentError !=
			// DQErrors.NO_ERROR) {
